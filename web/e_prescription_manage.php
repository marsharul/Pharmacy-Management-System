...
<!--                            <button class="btn-search btn border border-secondary btn-md-square rounded-circle icons-bg me-4" data-bs-toggle="modal" data-bs-target="#searchModal"><i class="fas fa-search text-color"></i></button>-->
        ...
        <div class="container-fluid py-5 bgcolorbody">
            <div class="container py-5">
                ...
                $sql = "SELECT p.*,ps.StatusName FROM prescriptions p INNER JOIN prescription_status ps ON ps.Id=p.StatusId WHERE CustomerId='$CustomerId' ";
                $result = $db->query($sql);
                ?>
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($row['StatusName'] == 'Pending') { ?>

                                            <?php } else { ?>
                                                <a href="quotation_cart.php?id=<?= $row['Id'] ?>" class="btn btn-primary"><i class="far fa-eye"></i> View Quotation Details</a></td>
                                        <?php } ?>
                                    </tr>
                                    <?php
                                }
