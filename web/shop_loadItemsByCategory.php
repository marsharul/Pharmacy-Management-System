...
//      echo$sql = "SELECT s.Id,s.Qty,s.Qty-s.IssuedQty as AvailQty,s.RetailPrice,i.ItemName,i.UploadPicture,d.Form FROM stocks s INNER JOIN items i ON i.Id=s.ItemId INNER JOIN dosage_form d ON d.Id=i.FormId";
    ...
    <div class="col-lg-9" id="product_grid"> <!-- Product Grid Tiles start -->
        <div class="row g-4 justify-content-center">
            ...
            $db = dbConn();
// echo$sql = "SELECT s.Id,s.Qty,s.Qty-s.IssuedQty as AvailQty,s.RetailPrice,i.ItemName,i.UploadPicture,d.Form FROM stocks s INNER JOIN items i ON i.Id=s.ItemId INNER JOIN dosage_form d ON d.Id=i.FormId";
            $sql = "SELECT s.Id,SUM(s.Qty) AS Qty,SUM(s.Qty-s.IssuedQty) AS AvailQty,s.RetailPrice,i.ItemName,i.UploadPicture,i.Description,i.ItemIssue,d.Form FROM stocks s INNER JOIN items i ON i.Id=s.ItemId INNER JOIN dosage_form d ON d.Id=i.FormId WHERE i.CategoryId='$categoryId' GROUP BY ItemId,RetailPrice";
            $result = $db->query($sql);
